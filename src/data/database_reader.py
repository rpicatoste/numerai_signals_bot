...
class DataBaseReader:
    ...
    def get_data(self):
        ...
        print(
            f"Ticker data loaded:     {len(ticker_data): 10}, cols: {ticker_data.shape[1]}"
        )
        print(
            f"Ticker universe loaded: {len(self.ticker_universe): 10}, cols: {self.ticker_universe.shape[1]}"
        )

        # Keep only tickers in the numerai universe
        tickers_idx = ticker_data.bloomberg_ticker.isin(
            self.ticker_universe["bloomberg_ticker"]
        )
        ticker_data = ticker_data[tickers_idx]
        print(
            f"Ticker data useful:     {len(ticker_data): 10}, cols: {ticker_data.shape[1]}"
        )
