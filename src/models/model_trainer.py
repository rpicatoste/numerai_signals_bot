...
class ModelTrainer:
    ...
    def train_model(self):
        ...
        self.model.fit(
            self.feat_gen.train[self.feature_names], self.feat_gen.train[TARGET_NAME]
        )
    ...
    def predict(self, output_dir=None):
        ...
        self.feat_gen.tournament = self.feat_gen.tournament.dropna(
            subset=self.feature_names
        )
        self.feat_gen.tournament[PREDICTION_NAME] = self.model.predict(
            self.feat_gen.tournament[self.feature_names]
        )
        ...
        diagnostic_df = diagnostic_df.drop_duplicates(
            subset=["bloomberg_ticker", "friday_date"], keep="first"
        )
        diagnostic_df[
            ["bloomberg_ticker", "friday_date", "data_type", "signal"]
        ].reset_index(drop=True).to_csv(example_signal_output_path, index=False)
